...
class EditProgressView extends StatefulWidget {
  ...
  const EditProgressView({
    super.key,
    required this.entry,
  });

  @override
  State<EditProgressView> createState() => _EditProgressViewState();
}
...
class _EditProgressViewState extends State<EditProgressView> {
  ...
  late final TextEditingController _fatMassController;
  ...
  void _initializeControllers() {
    _selectedDate = DateTime.parse(widget.entry.measurementDate);

    _weightController =
        TextEditingController(text: widget.entry.weight.toString());
    _notesController = TextEditingController(text: widget.entry.notes ?? '');

    final m = widget.entry.measurements;
    _chestController = TextEditingController(text: m.chest?.toString() ?? '');
    _leftArmController =
        TextEditingController(text: m.leftArm?.toString() ?? '');
    _rightArmController =
        TextEditingController(text: m.rightArm?.toString() ?? '');
    _waistController = TextEditingController(text: m.waist?.toString() ?? '');
    _hipsController = TextEditingController(text: m.hips?.toString() ?? '');
    _leftThighController =
        TextEditingController(text: m.leftThigh?.toString() ?? '');
    _rightThighController =
        TextEditingController(text: m.rightThigh?.toString() ?? '');

    final b = widget.entry.bodyComposition;
    _fatMassController =
        TextEditingController(text: b.fatMass?.toString() ?? '');
    _muscleMassController =
        TextEditingController(text: b.muscleMass?.toString() ?? '');
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          Obx(() => TextButton(
                onPressed: controller.isUpdating.value ? null : _updateProgress,
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 4),
          Text(
            'Optional - Update measurements to track changes',
            style: Get.textTheme.bodySmall?.copyWith(
              color: AppTheme.textMuted,
            ),
          ),
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 4),
          Text(
            'Optional - Update body fat and muscle mass',
            style: Get.textTheme.bodySmall?.copyWith(
              color: AppTheme.textMuted,
            ),
          ),
            ...
            SizedBox(
              ...
              child: ListView.builder(
                ...
                itemBuilder: (context, index) {
                  final image = widget.entry.images[index];
                      ...
                      children: [
                        ...
                        Positioned(
                          ...
                          child: GestureDetector(
                            onTap: () => _toggleImageDeletion(image.id),
                            child: Container(
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 4),
          Text(
            'Optional - Update notes about your progress',
            style: Get.textTheme.bodySmall?.copyWith(
              color: AppTheme.textMuted,
            ),
          ),
  ...
  Future<void> _updateProgress() async {
    ...
    final success = await controller.updateProgressEntry(
      ...
      newImages: controller.selectedImages.isNotEmpty
          ? controller.selectedImages
          : null,
      deleteImageIds: _imagesToDelete.isNotEmpty ? _imagesToDelete : null,
    );
